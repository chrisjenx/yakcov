...
The library is published against one or more Compose Multiplatform versions in parallel. `compose-releases.toml` (repo root) is the single source of truth: each `[section]` is a channel defining `compose` + `compose-material3` (required) and optional `kotlin`/`xcode` overrides. Both channels are active: `[stable]` rides Compose 1.11.1, and `[next]` (pre-release) rides Compose 1.12.0-alpha01 on Kotlin 2.4.0. Retire a channel by commenting out its section; re-enable by uncommenting. `gradle/libs.versions.toml` holds the working defaults; channels override them at release/CI time. (Deeper release-script docs: `RELEASE_SCRIPT.md`.)
...
- **Toolchain coupling.** AGP and the Gradle wrapper are **single, shared** values (channels override only `compose`/`compose-material3`/`kotlin`), so the most demanding channel sets the floor for the whole repo. Advancing `[next]` to a new Compose alpha often drags AGP/Gradle/compileSdk with it — its androidx artifacts gate on them (e.g. Compose 1.12 required AGP ≥ 9.1.0 / compileSdk 37). Current floor: **AGP 9.1.1, Gradle 9.3.1, compileSdk/targetSdk 37**. Before bumping, check the Kotlin↔Gradle↔AGP support matrix (kotlinlang.org/docs/gradle-configure-project); stable's Kotlin 2.3.21 runs slightly beyond its tested AGP/Gradle range but builds cleanly.
